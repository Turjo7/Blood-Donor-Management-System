...
<tr><td class="lefttd">Gender</td><td><input name="r1" type="radio" value="male"  <?php if($gender=="Male"){ echo "checked" ;}  ?>>Male<input name="r1" checked="checked" type="radio" value="female" <?php if($gender=="Female"){ echo "checked" ;}  ?> />Female</td></tr>
        ...
        <tr><td class="lefttd"  style="vertical-align:middle"> Age</td><td><input type="text" name="t2"  required="required" pattern="[0-9]{2,2}" title="please enter only numbers  between 2 to 2 for age" value="<?php echo @$age;?>" /></td></tr>
         <tr><td>&nbsp;</td><td>&nbsp;</td></tr>
          <tr><td class="lefttd"  style="vertical-align:middle"> Mobile No</td><td><input type="text" name="t3" value="<?php echo @$mobile;?>"  pattern="^[0]{1}[1]{1}[3-9]{1}[0-9]{8}" title="please enter valid mobile no." /></td></tr>
         <tr><td>&nbsp;</td><td>&nbsp;</td></tr>
...
<?php
	...
	if(isset($_POST["sbmt"])) 
	{
		$cn=makeconnection();
					...
					$s="update donarregistration set name='" .$_POST["t1"]. "' ,Address='" .$_POST["t4"]. "',gender='" .$_POST["r1"]."' , age='" .$_POST["t2"]. "',mobile='" .$_POST["t3"]. "' where email='" .$_SESSION["email"]. "'";
		
$i=mysqli_query($cn,$s);
	mysqli_close($cn);
	echo "<script>location.replace('http://localhost/bloodbank/donor/index.php');</script>";
